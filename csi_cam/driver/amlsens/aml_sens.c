...
static int sensor_parse_power(struct amlsens *sensor)
{
	...
	sensor->gpio.camvdd_gpio = devm_gpiod_get_optional(sensor->dev,
												"camvdd",
												GPIOD_OUT_LOW);
	if (IS_ERR(sensor->gpio.camvdd_gpio)) {
		rtn = PTR_ERR(sensor->gpio.camvdd_gpio);
		dev_info(sensor->dev, "Cannot get camvdd gpio: %d\n", rtn);
	} else {
		gpiod_set_value_cansleep(sensor->gpio.camvdd_gpio, 1);
	}
