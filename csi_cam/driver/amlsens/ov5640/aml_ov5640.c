...
#ifdef OV5640_ONE_SENSOR_PROBE_TIME_OPT
// platform only have 1 mipi sensor.
// can use global variable ov5640_power_state to record power state.
#define POWER_OFF_STATE 0
#define POWER_ON_STATE  1

static int ov5640_power_state = POWER_OFF_STATE;
#endif
...
#endif
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		info.bps_m = ov5640_get_link_freq(ov5640)/1000000L;
		info.nlanes = ov5640->nlanes;
...
#ifdef OV5640_ONE_SENSOR_PROBE_TIME_OPT
	if (ov5640_power_state == POWER_ON_STATE)
		return 0;
#endif
...
#ifdef OV5640_ONE_SENSOR_PROBE_TIME_OPT
	ov5640_power_state = POWER_ON_STATE;
#endif
	return 0;
}
...
#ifdef OV5640_ONE_SENSOR_PROBE_TIME_OPT
	ov5640_power_state = POWER_OFF_STATE;
#endif
...
static int ov5640_register_subdev(struct ov5640 *ov5640)
{
	...
	v4l2_dev = &ov5640->v4l2_dev;
	snprintf(v4l2_dev->name, sizeof(v4l2_dev->name), AML_SENSOR_NAME, ov5640->index);

	rtn = v4l2_device_register(ov5640->dev, v4l2_dev);
	if (rtn < 0) {
		dev_err(ov5640->dev, "Could not register v4l2 device\n");
		goto err_return;
	}

	rtn = v4l2_device_register_subdev(v4l2_dev, &ov5640->sd);
	if (rtn < 0) {
		dev_err(ov5640->dev, "Could not register v4l2 sub device\n");
		goto err_return;
	}

	rtn = v4l2_device_register_subdev_nodes(v4l2_dev);
	if (rtn < 0) {
		dev_err(ov5640->dev, "Could not register v4l2 sub device\n");
		goto err_return;
	}
...
int ov5640_init(struct i2c_client *client, void *sdrv)
{
	 ...
	 */
#ifdef OV5640_ONE_SENSOR_PROBE_TIME_OPT
	ov5640->current_mode = &ov5640_modes_2lanes[0];
	ov5640->bpp = ov5640_formats[0].bpp;
	ov5640->nlanes = 2;
#else
